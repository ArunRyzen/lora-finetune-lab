...
| The exact prompts/replies during `lora eval` (set `LLM_DEBUG=1` — in your shell or in `.env`; a real env var takes precedence over the `.env` file) | [`debuglog.py`](../src/lora_finetune_lab/debuglog.py) + the `generate` methods in [`evaluation.py`](../src/lora_finetune_lab/evaluation.py) and [`gemini.py`](../src/lora_finetune_lab/gemini.py) |
